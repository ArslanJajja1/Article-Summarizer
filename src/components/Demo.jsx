            ...
            <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
                {allArticles.map((item,index)=>(
                    <div key={`link-${index}`} onClick={()=>setArticle(item)} className='link_card'>
                        <div className="copy__btn" onClick={()=>handleCopy(item.url)}>
        ...
        <div className='my-10 max-w-full flex justify-center items-center'>
            {
                ...
                : error ? (<p className='font-inter font-bold text-black text-center'>Error 😟 : Well that doesn't supposed to happen... <br/> <span className='font-satoshi font-normal text-gray-700'>{error?.data?.error}</span></p> ) :(
                    article.summary && (
